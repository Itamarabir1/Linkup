from sqlalchemy import Column, Integer, ForeignKey, DateTime, func, UniqueConstraint, Boolean, text, String
from sqlalchemy.orm import relationship
from sqlalchemy.dialects.postgresql import ENUM as PG_ENUM
from geoalchemy2 import Geography
from app.db.base import Base
from app.domain.bookings.enum import BookingStatus
from sqlalchemy import Column, Integer, ForeignKey, DateTime, func, UniqueConstraint, Boolean, text, String, Index
...
class Booking(Base):
    ...
    request_id = Column(Integer, ForeignKey("passenger_requests.request_id", ondelete="SET NULL"), nullable=True)
    ...
    status = Column(
        BookingStatusEnumType(BookingStatus, name="booking_status", create_type=False, values_callable=lambda x: [e.value for e in x]),
        nullable=False,
    ...
    def passenger_name(self) -> str | None:
        ...
        if self.passenger:
            name = getattr(self.passenger, "full_name", None) or getattr(self.passenger, "first_name", None)
            if name and str(name).strip():
                return str(name).strip()
        return None
