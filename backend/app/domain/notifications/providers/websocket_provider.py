...
class WebSocketProvider(BaseNotificationProvider):
    ...
    async def send(self, user: Any, config: Dict[str, Any], context: Dict[str, Any]) -> None:
        ...
        payload = {
            ...
            "ride_id": str(context.get("ride_id", "")), # המרה ל-string למניעת בעיות JSON
            "data": {
                "color": context.get("color") or config.get("color", "green"),
                "status": context.get("status") or config.get("status", "updated"),
                "message": context.get("message") or config.get("message", "עדכון נסיעה"),
