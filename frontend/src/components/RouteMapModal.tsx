...
export default function RouteMapModal({ data, onClose }: RouteMapModalProps) {
  ...
  // מפתח: קודם מ-env (אם הוגדר בפרונט), אחרת מהבקאנד – אותו GOOGLE_MAPS_API_KEY שב-.env של הבקאנד (Geocoding + Directions)
  useEffect(() => {
  ...
  // טעינת סקריפט Google Maps כשהמפתח זמין
  useEffect(() => {
  ...
  // ציור המפה כשהנתונים והסקריפט מוכנים (רק אחרי שה-container ב-DOM – מונע IntersectionObserver.observe לא-Element)
  useEffect(() => {
    ...
    // דחייה ל-frame הבא כדי שה-container יהיה ב-DOM וב-layout לפני ש־Maps משתמש ב־IntersectionObserver
  ...
  return (
    <div className="route-map-backdrop" role="dialog" aria-modal="true" aria-label="תצוגת מסלול על המפה">
      <div className="route-map-modal">
        <div className="route-map-header">
          <h2 className="route-map-title">תצוגה מקדימה: {data.summary || 'מסלול'}</h2>
          <button
            type="button"
        ...
        {resolvedKey === null && !loadError && (
          <p className="route-map-loading">טוען מפתח מפה מהבקאנד...</p>
        )}
        {loadError && (
          <p className="route-map-error">{loadError}</p>
        )}
